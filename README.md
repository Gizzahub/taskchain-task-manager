...
사람과 코딩 에이전트가 같은 Git 저장소 안의 작업 상태를 안전하게 공유하도록
**소유권·중단 복구·실행 이력**을 제공하는 local-first task runtime입니다.
서버도 계정도 외부 에이전트 도구도 필요하지 않고, 보드는 `git diff`로 읽히는
마크다운 파일로 남습니다.
...
`plan-progress`는 plan이 선언한 자식 카드를 보드의 완료 인덱스와 대조해 세어 줍니다.
이 수는 **읽을 때마다 파생**되며 카드에 다시 기록하지 않습니다 — 파일에 남겨야 하는
소비자가 그 쓰기와 성공 여부 보고를 소유합니다. 세는 일을 카드 이동에 묶으면
「옮겨졌는데 세어지지 않은」 상태가 생기므로 일부러 묶지 않았습니다. 미확정 전이가
남은 카드는 아직 속한 zone이 없어 거부합니다.
...
기존 module 카드의 명시 채택과 module/category별 단일·일괄 생성은
[모듈 경로 채택](docs/module-adoption.md)을 따릅니다.
...
### 공개 표면은 stdout 바이트 스트림뿐입니다

이 저장소에는 `pkg/`가 없습니다. 결과 문서를 만드는 타입은 전부 `internal/` 아래이거나
`package main`이라, 다른 모듈에서 import할 수 **없습니다** — 문서가 약속을 미룬 것이 아니라
`internal/`이 Go 컴파일러가 외부 import를 거부하는 경로이기 때문입니다.

따라서 이 CLI가 소비자에게 지는 계약은 **명령이 stdout에 쓰는 바이트와 exit code뿐**입니다.
타입 이름·필드 순서·Go 구조체 모양은 계약이 아니며 예고 없이 바뀝니다. 소비자는 JSON을
...
완료 의존성을 보존하는 보관과 supersede/force는 [archive 사용법](docs/archive.md)을 따릅니다.
